...
function setting() {
echo "---- Setting the mode, map and vehicle -----"
docker exec -u wayne apollo_dev_wayne /bin/bash /apollo/modules/autoset/auto_set.sh
}
...
docker exec -u wayne apollo_dev_wayne /bin/bash /apollo/scripts/bootstrap.sh stop
sleep 3
docker exec -u wayne apollo_dev_wayne /bin/bash /apollo/scripts/bootstrap.sh
sleep 5
}
...
docker exec -u wayne apollo_dev_wayne /bin/bash /apollo/modules/autoset/auto_modules.sh
echo "===============Turn all of the modules on===================="
}

#Turn the modules off
function set_modules_off() {
docker exec -u wayne apollo_dev_wayne /bin/bash /apollo/modules/autoset/auto_stop_modules.sh
}
